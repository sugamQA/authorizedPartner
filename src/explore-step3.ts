...
(async () => {
  ...
  // Click regions by coordinates
  const regionNames = ["Australia", "India", "Nepal"];
  ...
  for (const svc of services) {
    const checkbox = page.locator(`button[role="checkbox"]`).filter({ hasText: svc }).first();
  ...
  apiCalls.slice(-10).forEach((c, i) => {
    const short = c.url.split('cloudedu.com.au').pop() || c.url.split('/').pop() || "";
    console.log(`${i + 1}. ${short}`);
    if (c.body && c.body.length < 400) console.log(`   ${c.body}`);
    if (c.response) console.log(`   → ${c.response.substring(0, 250)}`);
  });
